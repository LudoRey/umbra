...
def main(
    ...
    filepath_to_header = {p: imageio.read_header(p) for p in imageio.list_files(sun_stacks_dir, extensions=imageio.extensions.FITS)}
    ...
    moon_header = imageio.read_header(os.path.join(moon_registered_dir, filename))
    ...
    img_y, header_y = imageio.read(grouped_filepaths[group_name][0])
    ...
    imageio.write(os.path.join(sun_hdr_dir, f"weights_{group_name}.fits"), weights, None)
    ...
    pedestal = header_y.get("PEDESTAL")
    if pedestal is not None and isinstance(pedestal, (int, float)):
        img_y = np.maximum(img_y - pedestal / 65535, 0)
        header_y.remove("PEDESTAL")
    img_y = img_y * ref_scaling_factor / scaling_factor
    ...
    for group_name in reversed(list(grouped_filepaths.keys())[:-1]):
        # Read image to fit
        img_x, header_x = imageio.read(grouped_filepaths[group_name][0])
        ...
        pedestal = header_x.get("PEDESTAL")
        if pedestal is not None and isinstance(pedestal, (int, float)):
            img_x = np.maximum(img_x - pedestal / 65535, 0)
            header_x.remove("PEDESTAL")
        img_x = img_x * ref_scaling_factor / scaling_factor
