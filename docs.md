...
- `low_threshold`, `high_threshold` : pixels above `high_threshold` or below `low_threshold` are rejected. Decrease `low_threshold` or increase `high_threshold` if brightness equalization fails due to insufficient overlap. Decrease `high_threshold` if saturated pixels leak through.
